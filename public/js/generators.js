// Builds Fabric.js group objects for calendars, checklists, and daily schedules.
const Generators = {
  ...
  buildCalendar({ view, monthValue, style }) {
    ...
    if (view === 'week') {
      ...
      days.forEach((d, i) => {
        ...
        objects.push(new fabric.Text(d, { left: i * colWidth + 8, top: 6, fontSize: 14, fontWeight: 'bold', fontFamily: 'Helvetica' }));
      });
      const group = new fabric.Group(objects, { left: 60, top: 60 });
      return group;
    }
    ...
    objects.push(new fabric.Text(title, { left: 0, top: 0, fontSize: 20, fontWeight: 'bold', fontFamily: 'Helvetica' }));

    const dayLabels = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
    dayLabels.forEach((d, i) => {
      objects.push(new fabric.Text(d, { left: i * cellW + 4, top: 34, fontSize: 11, fill: '#666', fontFamily: 'Helvetica' }));
    });
  ...
  buildChecklist({ rows, cols, title }) {
    ...
    objects.push(new fabric.Text(title || 'Checklist', { left: 0, top: 0, fontSize: 18, fontWeight: 'bold', fontFamily: 'Helvetica' }));
    ...
    if (cols <= 1) {
      ...
      for (let i = 0; i < rows; i++) {
        ...
        objects.push(new fabric.Text(`Habit ${i + 1}`, { left: 0, top: y, fontSize: 12, fontFamily: 'Helvetica' }));
        for (let c = 0; c < cols; c++) {
          objects.push(new fabric.Rect({
  ...
  buildSchedule({ startHour, endHour }) {
    ...
    for (let h = startHour; h < endHour; h++) {
      ...
      objects.push(new fabric.Text(label, { left: 0, top: y + 6, fontSize: 11, fill: '#666', fontFamily: 'Helvetica' }));
